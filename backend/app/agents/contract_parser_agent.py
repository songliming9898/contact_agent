...
**解析要求**：
1. 合同金额统一为人民币"元"单位（如合同写"50万元"，则 total_amount 为 500000.00）
2. 日期格式统一为 YYYY-MM-DD
3. 如果合同中没有某个字段，该字段填 null，不要编造
4. 付款方式识别：一次性付款 / 分期付款 / 按里程碑付款
5. 特别关注：尾款金额、尾款支付条件、试用期条款、售后服务条款
6. 提取关键条款摘要（验收标准、违约责任、知识产权等）
7. software_products 识别合同中涉及的软件产品名称和版本
8. hardware_products 识别合同中涉及的硬件产品（名称/型号/数量/单位）
